...
def load_pytorch_weights(model: ADTOFFrameRNN, weights_path: str, strict: bool = False) -> ADTOFFrameRNN:
    # Local change (see NOTICE.md): weights_only=True. The upstream call
    # unpickles arbitrary objects, which torch 2.5 warns about; this file is
    # one we ship ourselves and it holds nothing but tensors.
    ...
    # Local change (see NOTICE.md): upstream prints a warning and carries on
    # with randomly initialized layers for any weight it couldn't find. That
    # produces a model that runs, returns confident-looking numbers and is
    # nonsense - the single worst way for this to fail. Raise instead.
    if missing_keys:
        raise RuntimeError(f"ADTOF weights are missing layers: {sorted(missing_keys)}")
    return model
